...
/* ============================================================
   SECTION: SERVICE CUSTOMIZER MONITOR & CONTROLS
   ============================================================ */

// 1. SHOW/HIDE MONITOR: Runs every time the 'FrameText' iframe loads a new page
$('#FrameText').on('load', function () {
  try {
    var url = this.contentWindow.location.href.toLowerCase();
    var panel = $('#service-customizer-panel');

    // Check for specific service types (Liturgy, Matins, Vespers)
    if (url.includes('/li1/') || url.includes('/ma1/') || url.includes('/ve1/')) {
      panel.removeClass('hidden').show();
      console.log("Customizer: Supported service detected. Panel visible.");
    } else {
      panel.addClass('hidden').hide();
      console.log("Customizer: Non-customizable page. Panel hidden.");
    }
  } catch (e) {
    // This catch handles 'cross-origin' errors if you load an external site
    $('#service-customizer-panel').hide();
  }
});

// 2. MINIMIZE/EXPAND TOGGLE: Handles the sliding animation
function togglePanelSize() {
  var panel = document.getElementById('service-customizer-panel');
  var btn = document.getElementById('service-customizer-min-btn');

  if (panel) {
    // Toggles the 'minimized' CSS class
    panel.classList.toggle('minimized');

    // Update the button icon based on state
    if (panel.classList.contains('minimized')) {
      btn.innerHTML = "&#9634;"; // Square (Restore)
    } else {
      btn.innerHTML = "_";        // Dash (Minimize)
    }
  } else {
    console.error("Customizer Error: Could not find panel ID.");
  }
}

/* ============================================================
   FUNCTION: refreshServiceFrame
   Purpose: Reloads the Liturgy/Service frame without refreshing dcs.html
   ============================================================ */
function refreshServiceFrame() {
  var frame = document.getElementById('FrameText');
  if (frame) {
    // This reloads only the internal frame
    frame.contentWindow.location.reload();
    console.log("Customizer: Service frame refreshed.");
  } else {
    console.error("Customizer Error: Could not find FrameText to refresh.");
  }
}

/* ============================================================
   SECTION: INTERNAL CUSTOMIZER LOGIC
   ============================================================ */

// 1. CATEGORY SWAP: Switches between Parish and Bishop modes
function setServiceCategory(type) {
  ...
  if (type === 'hierarchical') {
    ...
    hierSection.style.display = 'block';
  } else {
