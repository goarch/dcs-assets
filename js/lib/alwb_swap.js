...
/**
 * performRowSwap
 * Triggered on document.ready. Checks the title for valid Matins endings,
 * then hot-swaps the content between .brc_01 and .erc_01 with the Paschal source.
 */
async function performRowSwap() {
  const pageTitle = document.title;
  const validEndings = ['.ma', '.ma2', '.ma3', '.ma4', '.ma5', '.ma6', '.ma9'];

  // Only run if the document title indicates a Matins service
  if (validEndings.some(ending => pageTitle.endsWith(ending))) {
    console.log(`Matched Matins title: ${pageTitle}. Initiating row swap...`);

    const sourceURL = 'https://dcs.goarch.org/test/dcs/h/b/matinsordinary/paschal/gr-en/index.html';

    try {
      // 1. Locate the markers in the current document
      const targetStart = document.querySelector('.brc_01')?.closest('tr');
      const targetEnd = document.querySelector('.erc_01')?.closest('tr');

      if (!targetStart || !targetEnd) {
        console.log("performRowSwap: Markers not found on this page.");
        return;
      }

      const tbody = targetStart.closest('tbody') || targetStart.closest('table');

      // 2. Fetch the source content
      const response = await fetch(sourceURL);
      if (!response.ok) throw new Error(`Fetch failed for ${sourceURL}`);

      const htmlText = await response.text();
      const sourceDoc = new DOMParser().parseFromString(htmlText, 'text/html');

      // 3. Locate markers in the source file
      const allSourceRows = Array.from(sourceDoc.querySelectorAll('tr'));
      const sIdx = allSourceRows.findIndex(row => row.querySelector('.brc_01'));
      const eIdx = allSourceRows.findIndex(row => row.querySelector('.erc_01'));

      if (sIdx === -1 || eIdx === -1) {
        console.error("performRowSwap: Markers missing in source document.");
        return;
      }

      const newRows = allSourceRows.slice(sIdx, eIdx + 1);

      // 4. Determine current positions in the live DOM
      const targetRowsArray = Array.from(tbody.rows);
      const startPos = targetRowsArray.indexOf(targetStart);
      const endPos = targetRowsArray.indexOf(targetEnd);
      ...
      // Insert the new Paschal content
      newRows.forEach((row, index) => {
        const importedRow = document.importNode(row, true);
        const insertIdx = startPos + index;
        tbody.insertBefore(importedRow, tbody.rows[insertIdx] || null);
      });

      console.log("performRowSwap: Content injected.");

      // 1. Convert classes to IDs for the new rows
      if (typeof convertClassToId === "function") {
        convertClassToId();
      }

      // 2. Re-initialize collapsible logic for "Show Prayers" etc.
      if (typeof initCollapsibleRows === "function") {
        initCollapsibleRows();
      }

    } catch (err) {
      console.error("performRowSwap Error:", err);
    }
  }
}
